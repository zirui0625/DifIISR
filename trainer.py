...
class TrainerBase:
    ...
    def init_logger(self):
        ...
        if self.rank == 0 and hasattr(self, 'ema_rate'):
            ema_ckpt_dir = save_dir / 'ema_checkpoints'
...
class TrainerDifIR(TrainerBase):
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            tt = torch.randint(                         # generate timestep
...
class TrainerDifIISR(TrainerDifIR):
    def __init__(self, configs):
        super().__init__(configs)
    ...
    def training_step(self, data):
        if hasattr(self.base_diffusion, "set_current_iters"):
            self.base_diffusion.set_current_iters(self.current_iters)
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            compute_losses = functools.partial(
                ...
                micro_data['lq'],
                tt,
                use_perceptual=self.configs.guidance.use_perceptual,
                W_per=self.configs.guidance.W_per,
                use_visual=self.configs.guidance.use_visual,
                W_vis=self.configs.guidance.W_vis,
                first_stage_model=self.autoencoder,
                model_kwargs=model_kwargs,
                noise=noise,
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            _saved_use_guidance = self.base_diffusion.use_guidance
            self.base_diffusion.use_guidance = False
            ...
            try:
                ...
                for ii, data in enumerate(self.dataloaders[phase]):
                    ...
                    _, _, h_orig, w_orig = im_lq.shape
                    pad_unit = 128
                    pad_h = (pad_unit - h_orig % pad_unit) % pad_unit
                    pad_w = (pad_unit - w_orig % pad_unit) % pad_unit
                    ...
                    if pad_h > 0 or pad_w > 0:
                        sf = self.configs.degradation.sf
                        results = results[:, :, :h_orig * sf, :w_orig * sf]
                    ...
                    if (ii + 1) % self.configs.train.log_freq[2] == 0:
                        ...
                        im_lq_orig = im_lq[:, :, :h_orig, :w_orig] if (pad_h > 0 or pad_w > 0) else im_lq
                        x3 = vutils.make_grid(im_lq_orig, normalize=True)
                        if self.configs.train.save_images:
                            util_image.imwrite(
                                   x3.cpu().permute(1,2,0).numpy(),
                                   self.image_dir / phase / f"lq_{self.log_step_img[phase]:05d}.png",
                ...
                self.base_diffusion.use_guidance = _saved_use_guidance
